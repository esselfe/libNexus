...
//	printf ("key: %d :%c:\n", key, key);
	if (key == 27) {	// Escape
		menuHideAll ();
		drawnMenu->drawn = drawnMenu->drawn?0:1;
		//printf ("\n");
...
//	printf ("mod: %d\n", mod);
	...
	if (mod == GLUT_ACTIVE_CTRL) { printf ("ctrl + :%c: | %d\n",key,key);
		...
		case 'i':
			...
			break;
		case 'l':
			options.showFloorLines = options.showFloorLines ?0:1;
			break;
		case 'm':
...
void specialKeysDown (int key, int x, int y) {
	...
	if (mode == MODE_RACE) {
		...
		switch (key) {
				case GLUT_KEY_F1:
					printf ("mode is #%d is %s\n", mode, modeStringByModeID(mode));
					break;
				case GLUT_KEY_F5:
					if (frameSleepTime >= 10000) frameSleepTime -= 10000;
					if (frameSleepTime <= 0) frameSleepTime = 0;
				...
				case GLUT_KEY_F6:
					...
					break;
			case GLUT_KEY_F7:
				if (deltaStep >= 5.0) deltaStep = 4.9;
				deltaStep -= 0.01;
				if (deltaStep <= 0.0)
					deltaStep = 0.01;
				printf ("deltaStep = %.2f\n", deltaStep);
...
void hoverObj (int x, int y) {
	...
	glLoadIdentity ();
	gluPickMatrix ((double)x, (double)viewport[3]/1.0 - (double)y, 1,1, viewport);
	...
	/*printf ("hoverObj(): %d hits: ", hits);
	int tcnt = 0;
	if (hits != 0) while (1) {
		printf (" %d", selectBuf[tcnt+3]);
		++tcnt;
		if (tcnt >= hits) break;
	} printf ("\n");
	*/
	if (hits > 0) {
		...
		while (1) { // for all hits
			...
			if (type == SID_TYPE_PLANE) {
				...
				printf ("events:hoverObj(): SID type not defined\n"); }
			cnt += 4;
			if (cnt/4 >= hits) break;
		}
		if (once64) {
...
void selectMenu (int x, int y) {
	...
	if (hits > 0) {
		//printf ("selectMenu(): %d hits\n", hits);
		if (debug.events) { printf ("*"); fflush (stdout); }
	}
	else {
		//printf ("selectMenu(): no hits\n");
		menuHideAll ();
	...
	if (hits == 1) {
		...
		struct Entry *entry = entryByID2 (__FILE__,__LINE__,(char *)__FUNCTION__,selectBuf[3]);
		if (entry == NULL) {
			...
			//printf ("selectMenu(): entry #%d :%s:\n", entry->selectID, entry->label);
			switch (selectBuf[3]) {
				case ENTRY_EXIT:
					printf ("\n");
					exitNexus (STATE_EVENT, STATE_OK, __FUNCTION__);
				...
				case ENTRY_OPTIONS_SHOWFPS:
					...
					break;
				case ENTRY_OPTIONS_SHOWTERMFPS:
					options.showTermFPS = options.showTermFPS ? 0 : 1;
					//entry = entryByID (ENTRY_OPTIONS_SHOWFPS);
					//if (entry != NULL) entry->enabled = entry->enabled ? 0 : 1;
					entry->enabled = options.showTermFPS;
					break;
				...
				*/ //case ENTRY_SUBMENU_FONT:
				...
				case ENTRY_MODE_BUFFER:
					NLsetMode (MODE_BUFFER);
					break;
				case ENTRY_MODE_GAME:
					NLsetMode (MODE_GAME);
					break;
				...
				case ENTRY_EDITOR_DEBUG_SHOWPOINT:
					...
					addMsg (4, msgbuf2);
					break;
				case ENTRY_EDITOR_DEBUG_SHOWLINES:
					debug.showMeshLines = debug.showMeshLines?0:1;
					entry->enabled = debug.showMeshLines;
					sprintf (msgbuf2, "Set debug.showPoint %s", debug.showPoint ? "on":"off");
					addMsg (4, msgbuf2);
					break;
...
void hoverMenu (int x, int y) {
	...
	hits = glRenderMode (GL_RENDER);
	setPerspectiveProjection ();
//	if (hits > 0) printf ("hoverMenu(): %d hits\n", hits);
...
void mousePassiveMotionFunc (int x, int y) {
	cursorX = x;
	cursorY = y;
	if (!mainMenuList.rootMenu->drawn && x <= 5.0 
	&& y > (winHeight -40.0) - (mainMenuList.rootMenu->height)) {//lastEntry->rank*ENTRY_SIZEY)) {
